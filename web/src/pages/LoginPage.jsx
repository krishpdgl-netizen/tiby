...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    ...
    // Only request email+profile — Gmail scope needs Google Cloud Console setup
      ...
      <div style={{ width:'100%', maxWidth:360 }}>
        ...
        {sent ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}>
            ...
            <div style={{ fontSize:13.5,color:'#6b7280' }}>Magic link sent to <strong>{email}</strong></div>
            <button onClick={()=>setSent(false)} style={{ marginTop:16,fontSize:13,color:'#6b7280',background:'none',border:'none',cursor:'pointer' }}>
              Try a different email
            </button>
          </div>
        ) : mode==='choose' ? (
          <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
            <button onClick={handleGoogle} disabled={loading} style={{ width:'100%',padding:'12px 14px',border:'1px solid #e5e5e4',borderRadius:11,background:'#fff',cursor:'pointer',display:'flex',alignItems:'center',gap:10,fontSize:14,fontWeight:600,color:'#1a1a1a',opacity:loading?.6:1,fontFamily:'inherit' }}>
            ...
            <button onClick={()=>setMode('email')} style={{ width:'100%',padding:'12px 14px',border:'1px solid #e5e5e4',borderRadius:11,background:'#fff',cursor:'pointer',display:'flex',alignItems:'center',gap:10,fontSize:14,fontWeight:600,color:'#1a1a1a',fontFamily:'inherit' }}>
          ...
          <form onSubmit={handleMagicLink} style={{ display:'flex',flexDirection:'column',gap:10 }}>
            ...
            <button type="submit" className="t-btn t-btn-primary" disabled={loading||!email.trim()}>
        ...
        <p style={{ fontSize:11.5,color:'#9ca3af',textAlign:'center',marginTop:20,lineHeight:1.6 }}>
          By signing in you allow Tiby to send emails on your behalf.
        </p>
      </div>

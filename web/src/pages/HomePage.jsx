...
export default function HomePage({ user }) {
  ...
  async function handleCommand(text){
    ...
    try{
      ...
      console.error(e)
      addMsg('tiby',e?.response?.status===429?'I\'m receiving too many requests right now. Try again in a moment.':'Something went wrong — check your connection and try again.')
    }
  ...
  async function startRecording(){
    try{
      ...
      mr.onstop=async()=>{
        ...
        const blob=new Blob(chunksRef.current,{type:'audio/webm'}); setThinking(true)
        try{
